...
N, T, n_ipca_rff = 1000, 720, 36

include_market = False # dummy to include market in DKKM
...
alpha_lst =  [0, 0.01, 0.05, 0.1, 1] # B-J shrinkage for DKKM
if model =='gs':
    alpha_lst = [0, 0.0000001, 0.0000005, 0.000001, 0.000005, 0.00001, 0.00005, 0.0001, 0.0005, 0.001, 0.005, 0.01, 0.05, 0.1, 1]
...
def run_panel():
    ...
    def generate_rff_panel(i):
        ...
        del gamma
        return W, dkkm.factors(panel=panel, W=W, n_jobs=n_jobs, start=start, end=end, model=model) 
    ...
    def run_month(month):
        ...
        for method in ["taylor", "proj"]:
            ...
            new_row.to_csv('port_model_' + model + '.csv', mode='a', header=not os.path.exists('port_model_' + model + '.csv'), index=False)
        ...
        dkkm_results = {}
        dkkm_ports = {
                        (a, n, i): np.empty((len(data),)) for a in alpha_lst for n in nfeatures_lst for i in range(nmat)
                    }
        

        half = max_features // 2
        for i, tpl in enumerate(dkkm_lst):
            W, frets = tpl
            
            # now compute portfolios for dkkm
            for nfeatures in nfeatures_lst:
                ...
                print(f"shape(factor_weights) = {factor_weights.shape}")
                print(
                    f"nfeatures = {nfeatures}, len(nf_indx)  = {len(nf_indx)}, len(factor_weights_columns) = {len(factor_weights.columns)}"
                    )
                factor_weights.columns = [str(ind) for ind in nf_indx]
                if include_market:
                    factor_weights['mkt_rf'] = fama.fama_french(data[chars], mve=data.mve)[:, -1]
                for alpha in alpha_lst:
                    port_of_factors = dkkm.mve_data(frets.iloc[:, nf_indx], month, nfeatures*alpha, ff_rets.iloc[:, -1] if include_market else None)
                    weights_on_stocks = factor_weights @ port_of_factors
                    dkkm_ports[(alpha, nfeatures, i)] = weights_on_stocks
                    weights = np.zeros((N,))
                    weights[keep_this_month] = weights_on_stocks
                    dat = {
                        'month':month,
                    ...
                    new_row = pd.DataFrame([dat])
                    new_row.to_csv('port_dkkm_' + model + '.csv', mode='a', header=not os.path.exists('port_dkkm_' + model + '.csv'), index=False)
        
        # compute results
        for alpha in alpha_lst: 

            # compute results for dkkm
            for nfeatures in nfeatures_lst:
                for i in range(nmat):
                    weights_on_stocks = dkkm_ports[(alpha, nfeatures, i)] #dkkm_ports[(alpha, nfeatures)].mean(axis=1)
                    ...
                    errs = rp - second_moment @ weights_on_stocks
                    hjd = errs @ second_moment_inv @ errs
                    dkkm_results[(alpha, nfeatures, i)] = [stdev, mean, xret, hjd]

        dkkm_results = pd.DataFrame(dkkm_results).T
        dkkm_results.columns = ["stdev", "mn", "xret", "hjd"]
        dkkm_results.index.names = ["alpha", "nfeatures", "mat"]
        dkkm_results = dkkm_results.reset_index()
        dkkm_results["month"] = month
        ...
        for m in ["ff", "fm"]:
            ...
            for alpha in alpha_lst_fama:
                ...
                new_row = pd.DataFrame([dat])
                new_row.to_csv('port_fama_' + model + '.csv', mode='a', header=not os.path.exists('port_fama_' + model + '.csv'), index=False)
        ...
        for m in ipca_nfactors_lst:
            ...
            new_row = pd.DataFrame([dat])
            new_row.to_csv('port_ipca_' + model + '.csv', mode='a', header=not os.path.exists('port_ipca_' + model + '.csv'), index=False)
    ...
    #  delayed(run_month)(month) for month in range(start+360, end+1)
    
    #)        
    results_lst = [run_month(month) for month in range(start+360, end + 1)]
...
for iter in range(startiter, startiter + numiters):
    ...
    panel, model_results, dkkm_results, fama_results, tseries, model_premia, fm_rets, ff_rets, ipca_results, ipca_rets, sorted_ports = run_panel() #

    panel["iter"] = iter 
    panel_out = pd.concat((panel_out, panel))
