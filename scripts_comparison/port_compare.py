...
print('port_compare.py started at %s' % datetime.datetime.now())

####
# basic specifications

# econ models considered
econ_models = ['bgn1','kp1'] # HACK HACK HACK lop off the 1 for the results files below

# where the simulation data files live
results_dir = 'results_main/' # if dir, end with "/"
...
excel_name = 'port_compare.xlsx' # name of the Excel file (perhaps correspond with results_dir)
...
for em in econ_models:
    ...
    spec_cols[em]['dkkm'] = ['mat','nfeatures','alpha']
    
    # results files -- HACK HACK HACK lop off the number in em (last string char)
    results[em] = {}
    results[em]['model'] = pd.read_csv(results_dir + 'results_model_' + em[:-1] + '.csv') # taylor and proj based
    results[em]['fama'] = pd.read_csv(results_dir + 'results_fama_' + em[:-1] + '.csv') # ff and fm
    results[em]['ipca'] = pd.read_csv(results_dir + 'results_ipca_' + em[:-1] + '.csv') # ipca with K=1,2
    results[em]['dkkm'] = pd.read_csv(results_dir + 'results_dkkm_' + em[:-1] + '.csv') # dkkm with various features, alpha, 20 RFF panels
    ...
    tseries[em] = pd.read_csv(results_dir + 'results_tseries_' + em[:-1] + '.csv') # tseries combines
...
print(' done with data manipulations at %s' % (datetime.datetime.now()))
...
for em in econ_models:
    ...
    for k in [k for k in results[em].keys()]:
        ...
        def sdfreg(dfin): # regression dict calculation function to apply in the groupby
            r = sm.OLS(dfin['sdf_ret'], exog=dfin['xret']).fit()
    ...
    perform_measures[em]['dkkm avgmat'] =\
        perform_measures[em]['dkkm'].groupby(['nfeatures','alpha']).mean()
...
# for the same (nfeatures,alpha) pairs.
# We select the (nfeatures,alpha) pairs as those that do "best" by different measures
...
for em in econ_models:
    ...
    tmp = port[em]['dkkm'].drop('mat',axis=1).\
        set_index(['nfeatures','alpha']).\
            reset_index().set_index(['nfeatures','alpha','month']).sort_index().copy()
    ...
    tmp = (tmp.groupby(['nfeatures','alpha']).apply(descfcn)).unstack()
    ...
    # each (nfeatures,alpha) pair. Then we average them for the month
    ...
    tmp3 = (
        corr_weights[em]['dkkm'].T.groupby(['nfeatures','alpha']).mean())\
        .T.describe().T
...
dflist = []
for em in econ_models:
    # Find the (nfeatures,alpha) combos that appear in any best set
    y = list(set(
    ...
    any_best = list(set(
        [x[:2] for x in sum_stats_cscorr.sort_index().loc[
            pd.IndexSlice[em,y,:,:]].index.get_level_values('specification')]
        )) # just the (nfeatures,alpha) pairs appearing in any best
...
print(" ")
print("port_compare.py is still running to calculate persistence of the weights")
...
if write_summary_excel:
    ...
    with pd.ExcelWriter('persistence.xlsx', mode=exmode) as writer:
